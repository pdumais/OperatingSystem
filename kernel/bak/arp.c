...
{
    unsigned long ownMAC = net_getMACAddress();
    ...
    if (payload->to == 0x0000FFFFFFFFFFFF)
    {
        ...
        if (hwType==0x0100 && protocolType==0x0008)
        {
            if (operation==0x0100)
            {
                ...
                net_send(payload->from, 0x0100, 0x0608, (char*)&buf2[0], 28);    
            }
        }
...
void arp_query(unsigned int ip)
{
    SWAP4(ip);
    unsigned long ret;
    ...
    ret = net_send(0xFFFFFFFFFFFF, 0x0100, 0x0608, (char*)&buf[0], 28);
    pf("sent ARP query for %x\r\n",ip);
}

//WARNING: this parameter takes big-endian
unsigned long arp_getMAC(unsigned int ip)
{
